...
describe Move do
  describe '#follows_rules?' do
    context 'when one of a move\'s rules is broken by the board state' do
      subject(:illegal_move) { described_class.new(Position.parse('a1'), Position.parse('c1'), [Rules::COLLISION, Rules::FRIENDLY_FIRE] )}
    ...
    context 'when none of a move\'s rules are broken by the board state' do
      subject(:legal_move) { described_class.new(Position.parse('a1'), Position.parse('c1'), [Rules::COLLISION, Rules::FRIENDLY_FIRE] ) }
  ...
  describe '#==' do
    subject(:move) { described_class.new(Position.parse('a1'), Position.parse('a3'), [Rules::COLLISION]) }

    context 'when objects share the same from, to, target, and rules attributes' do
      it 'returns true' do
        other = described_class.new(Position.parse('a1'), Position.parse('a3'), [Rules::COLLISION])
